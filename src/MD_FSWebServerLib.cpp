...
bool AsyncFSWebServer::load_config()
{
  ...
  _config.TargetCT = jsonDoc["TargetCT"].as<long>();
  _config.debounceDelay = jsonDoc["debounceDelay"].as<long>();

  // config.connectionLed = jsonDoc["led"];
  //TODO: for Arrgh - retrieve and initialize MCC data - note: BSM/Battery data can change
...
bool AsyncFSWebServer::defaultConfig()
{
  ...
  {
    DEBUG_MSG("opened default config file\n");
  }
...
void AsyncFSWebServer::handleFileList(AsyncWebServerRequest *request)
{
  ...
  while (dir.next())
  {
    ...
    filename = String(entry.name()).substring(1);
  ...
  output += "]";
  DEBUG_MSG("Sending this file and directory list: \n\r%s\r\n", output.c_str());
                                      ...
                                      AsyncWebServerRequest *request)
{
  ...
  if (path.endsWith("/"))
    path += "index.htm";
  DEBUG_MSG("file extension .htm? %i\r\n", path.endsWith(".htm"));
      ...
      _fs->exists(path))
  {
    ...
    // }
    // if (path.endsWith(".htm") || path.endsWith(".js") || path.endsWith(".css"))
    // {
    //   response->addHeader("Content-Encoding", "gzip");
    // }
    ...
    AsyncWebServerRequest *request)
{
  ...
  if (!checkAuth(request))
    return request->requestAuthentication();
  DEBUG_MSG("Proces request from config_module.htm with %d Params\r\n", request->args());
                           ...
                           // }
  {
    for (uint8_t i = 0; i < request->args(); i++)
    {
      ...
      if (request->argName(i) == "debounceDelay")
      {
        _config.debounceDelay = request->arg(i).toInt();
        // DEBUG_MSG("_config.debounceDelay being saved to config.json: '%ld', returned value '%s'\n\r", _config.debounceDelay, request->arg(i).c_str());
        continue;
      }
    }
    save_config();
    ...
    AsyncWebServerRequest *request)
{
  ...
  values += "debounceDelay|" + (String)_config.debounceDelay + "|input\n";
  values += "brand| " + (String)PAGOI_CLIENT + "|div\n";
  request->send(200, "text/plain", values);
  DEBUG_MSG(__FUNCTION__);
  DEBUG_MSG("\r\n");
...
#define HIDE_SECRET
  ...
  on("/all", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    json += ", \"Humidity\":" + LastHumid;
    // json += ", \"TargetCT\":"+  String(_config.TargetCT);
    json += ", \"LastCT\":" + String(CycleTime);
    json += ", \"Sensor2GndState\":" + String(last_Sensor2Gnd_state);
    json += "}";
    request->send(200, "text/json", json);
    json = String();
  });
