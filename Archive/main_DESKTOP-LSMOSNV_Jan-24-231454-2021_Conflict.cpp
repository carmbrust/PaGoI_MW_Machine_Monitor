...
void ModuleIDFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
  ...
  display->drawString(0 + x, 26 + y, localIP_s);
...
void checkForUpdates()
{
  if (IpAddress2String(WiFi.localIP()) > "0.0.0.0")
  {
    ...
    // fwURL.concat(OTA_PATH); //TODO: use client as a variable
    // fwURL.concat("/");
    ...
    DEBUG_MSG("Checking Firmware version %s \n", fwVersionURL.c_str());
    ...
    if (httpCode == 200)
    {
      ...
      if (newVersion != APP_VERSION)
      {
        ...
        String fwImageURL = fwURL_generic;
